...
import { email_validation, full_name_validation, mobile_number_validation } from "../../src/utils/inputValidations";
import { ReactTestInstance } from "react-test-renderer";

jest.mock("../../src/hooks/useFetchUserInfoById.tsx");
const mockedUseFetchUserInfoById = useFetchUserInfoById as jest.MockedFunction<typeof useFetchUserInfoById>;
...
beforeEach(() => {
  const userAccountInfo = null;
    mockedUseFetchUserInfoById.mockReturnValue({userAccountInfo, isFetchingAccountInfo: false})
...
describe("AddNewContactScreen-component", () => {
  ...
  test("At initial render, PageLoadSpinner & ErrorPopUpModal components should be hidden", () => {
    const { toJSON, root, UNSAFE_root, UNSAFE_getAllByType, debug  } = renderedComponentInfo;
  ...
  test("At initial render, FullName, PhoneNumber & Email input fields should be empty", () => {
    ...
    const fullNameInputField = getByTestId(`textInput_${full_name_validation.id}`);
    ...
    const phoneNumberInputField = getByTestId(`textInput_${mobile_number_validation.id}`);
  ...
  test("After updating the 'FullName' input field, it should match with updated value", () => {
    ...
    fireEvent.changeText(fullNameInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    // input should match with updated value
    waitFor(() => expect(fullNameInputField?.props?.value).toBe(TEXT_TO_BE_UPDATED_WITH));
  });
  ...
  test("After updating the 'Mobile number' input field, it should match with updated value", () => {
    ...
    /** testing Full name */
    const mobileNumberInputField = getByTestId(`textInput_${mobile_number_validation.id}`);
    expect(mobileNumberInputField).toBeVisible();

    // setting up some value into test
    fireEvent.changeText(mobileNumberInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    // input should match with updated value
    waitFor(() => expect(mobileNumberInputField?.props?.value).toBe(TEXT_TO_BE_UPDATED_WITH));
  });
  ...
  test("After updating the 'E-mail' input field, it should match with updated value", () => {
    ...
    // setting up some value into test
    fireEvent.changeText(emailInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    // input should match with updated value
    waitFor(() => expect(emailInputField?.props?.value).toBe(TEXT_TO_BE_UPDATED_WITH));
  });
  ...
  test(`If the Full Name input is not matching the the validations, it's should show error `, async () => {
    ...
    let errorForFullNameInputField = queryByTestId(`errorMessage_${full_name_validation.id}`);
    expect(errorForFullNameInputField).toBeNull();
    
    const fullNameInputField = queryByTestId(`textInput_${full_name_validation.id}`);
    // setting up some value into test
    fireEvent.changeText(fullNameInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    
    errorForFullNameInputField = queryByTestId(`errorMessage_${full_name_validation.id}`);
  ...
  test(`If the Mobile Number input is not matching the the validations, it's should show error `, async () => {
    ...
    let errorForMobileNumber = queryByTestId(`errorMessage_${mobile_number_validation.id}`);
    expect(errorForMobileNumber).toBeNull();
    
    const mobileNumberInputField = queryByTestId(`textInput_${full_name_validation.id}`);
    // setting up some value into test
    fireEvent.changeText(mobileNumberInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    
    errorForMobileNumber = queryByTestId(`errorMessage_${full_name_validation.id}`);
  ...
  test(`If the Email input is not matching the the validations, it's should show error `, async () => {
    ...
    let errorForEmailInput = queryByTestId(`errorMessage_${mobile_number_validation.id}`);
    expect(errorForEmailInput).toBeNull();
    
    const emailInputField = queryByTestId(`textInput_${full_name_validation.id}`);
    // setting up some value into test
    fireEvent.changeText(emailInputField as ReactTestInstance, TEXT_TO_BE_UPDATED_WITH);
    
    errorForEmailInput = queryByTestId(`errorMessage_${full_name_validation.id}`);
